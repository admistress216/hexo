...
### 3. 开发composer包步骤
#### 3.1 GitHub 创建一个名 uploadfile 新仓库，并克隆至本地
```php
$ git clone git@github.com:anker1992/uploadfile.git
$ cd uploadfile
```
#### 3.2 初始化项目，生成composer.json文件
```php
/***** 1. 步骤 ******/
...
```
#### 3.4 构建项目
##### 3.41 新建uploadfile/src/UploadFile.php
```php
├─uploadfile                
│  ├─src                     
│  │  ├─UploadFile.php
...
##### 3.42 测试
- 终端下执行 composer install，这时会生成vendor目录，及其他文件
```php
yzm@Alert MINGW64 /i/phpstudy/WWW/uploadfile
$ composer install
```

- 新建uploadfile/test/UpploadFileTest.php、uploadfile/test/UpploadFile.html
```php
// UpploadFileTest.php
...
```
- 本地浏览器访问uploadfile/test/UpploadFile.html进行测试
![文件上传](/images/upload.gif)

#### 3.5 添加 README.md、LICENSE、.gitignore等文件，项目最终结构如下[我的github地址](https://github.com/anker1992/uploadfile)
```php
├─uploadfile                扩展包根目录
│  ├─src                    扩展包代码目录
│  │  ├─UploadFile.php
...
git push v1.0.0
```
#### 3.7 将 GitHub 上的包提交到 Packagist
- 首先要在 [Packagist](https://packagist.org) 上注册账号并登录（可以用 GitHub 直接登录）
- 点击顶部导航条中的 Summit 按钮
- 在输入框中输入 GitHub 上的刚才包地址，如：https://github.com/anker1992/uploadfile
...
cd laravel
composer require anker1992/uploadfile
```