...
命令格式：
    ...
    (1) -an: 去掉音频
    (2) -vn: 去掉视频
    (3) -acodec/c:a : 音频选项， 一般后面加copy表示拷贝/ 音频编码器,转为mp3使用libmp3lame值
    (4) -vcodec/c:v :视频选项，一般后面加copy表示拷贝
    (5) -cv: 设置视频编码器(外部编码器使用,内部也可使用此参数)
    (6) -ar + 44100/48000 : 设置音频采样率(如果不指定,默认为原视频的采样率)
    (7) -ab + 320k: 设置音视频比特率(默认为128k)
    (8) -ac + 2 : 设置音频声道(1:单声道,2:双声道,立体声)
    2、格式：
